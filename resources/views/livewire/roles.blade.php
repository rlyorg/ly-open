<div class="flex-col space-y-4">
    ...
    <x-table>
        <x-slot name="head">
            ...
            <x-table.heading  multi-column >Permissons</x-table.heading>
                    ...
                    <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $model->id }}">
                        ...
                        <x-table.cell>
                            @foreach($model->permissions as $permission)
                                <div class="text-cool-gray-900 font-medium">{{ $permission->name }} </div>
                            @endforeach
                        </x-table.cell>
    ...
    <form wire:submit.prevent="savePermissons" >
        <x-modal.dialog wire:model.defer="showEditModal">
            <x-slot name="title">Edit {{ $editing->name }}'s Permissons</x-slot>

            <x-slot name="content">
                <x-input.group for="permissons" label="Permissons" :error="$errors->first('permissons')">
                    @foreach($permissons as $label=>$value)
                    {{ $label }}
                    <x-input.checkbox wire:model="permissons.{{$label}}" :name="$value"/>
                    @endforeach
                </x-input.group> 
            </x-slot>
    ...
    <form wire:submit.prevent="save" >
        <x-modal.dialog wire:model.defer="showCreateModal">
            <x-slot name="title">Create A New Role</x-slot>
